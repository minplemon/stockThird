...
def uu_query_STK_SHAREHOLDER_FLOATING_TOP10(stock,pub_date = '2015-01-01'):
    """
    十大流通股东
    ...
    q = query(finance.STK_SHAREHOLDER_FLOATING_TOP10).filter(
        ...
        finance.STK_SHAREHOLDER_FLOATING_TOP10.pub_date > pub_date).limit(10)
    df = finance.run_query(q)
    return df
...
def uu_query_STK_HOLDER_NUM(stock,pub_date='2015-01-01'):
    """
    股东户数
    ...
    q = query(finance.STK_HOLDER_NUM).filter(finance.STK_HOLDER_NUM.code == stock,
                                             finance.STK_HOLDER_NUM.pub_date > pub_date).limit(10)
    df = finance.run_query(q)
    return df
